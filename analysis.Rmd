...
```{r}
github_url <- "https://raw.githubusercontent.com/DACSS-601/hw5-1/main/"
file_names <- c("fsi-2006.xlsx", "fsi-2007.xlsx", "fsi-2008.xlsx", "fsi-2009.xlsx", 
                ...
                "fsi-2022-download.xlsx", "FSI-2023-DOWNLOAD%20(1).xlsx")
all_data <- map_df(file_names, function(file) {
  url <- paste0(github_url, file)
  local_file <- gsub("%20", " ", file)
  download.file(url, local_file, mode = "wb", quiet = TRUE)
  read_excel(local_file) %>% mutate(Year = as.numeric(Year))
})
```

Explanation: Read each Excel file directly from GitHub raw content URL instead of local files.
